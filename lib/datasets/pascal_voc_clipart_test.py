...
class clipart_test(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'voc_' + year + '_' + image_set)
        print("这里clipart的测试，以源域voc的标签为准，共享10类")
        self._year = year
        ...
        #                  'sheep', 'sofa', 'train', 'tvmonitor')#uda训练测试设置
        self._classes = ('__background__',  # always index 0
                         'aeroplane', 'bicycle', 'bird', 'boat', 'bottle',  # uda源域私有
                         'bus', 'car', 'cat', 'chair', 'cow', 'diningtable', 'dog', 'horse', 'motorbike', 'person'  # 公有
                         )  # uda训练测试设置
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'clipart')
    ...
    def _load_pascal_annotation(self, index):
        ...
        """
        filename = os.path.join(self._data_path, 'Annotations', index.split()[0] + '.xml')
