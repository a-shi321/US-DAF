...
import math
from model.da_faster_rcnn.DA import _ImageDA,class_ImageDA
...
def BCEloss_margin(x_sigmoid, label):
    ...
    n = len(BCEloss)
    a = (BCEloss[:, 0] > 0.5).reshape(n, -1).float()
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes,
        ...
        source_label_ins = torch.cat([source_domain_label, source_small, source_middle, source_large], dim=1).cuda()
        ...
        target_label_ins = torch.cat([target_domain_label, target_small, target_middle, target_large], dim=1).cuda()
